...
that are pure code and therefore need nothing: the checker that proves it can fail, the
policy language refusing to learn a string filter, and the offline evidence reader
refusing a damaged bundle. **The full attack loop is deliberately not run there** — it
needs Vertex AI and a billing account of your own, and step 5 of the tutorial says so
...
2. **The claim and its evidence** → [Results](#results). **Read that section first: a
   sixty-run live batch completed on 2026-08-25, and the `Observed` column now carries
   figures for the rows that batch measured.** Every one of them prints `k=1` and the
   policy / approval-oracle split beside it. The rows the batch did not measure stay
   dashed, each with its reason in the cell, and **the row that fails is the exclusion
   rate**: it is over the ceiling in 51 of the 60 runs.
