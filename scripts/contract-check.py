...
#
# WHY `unenforced` IS AN ESCAPE AND WHY IT IS NOT A RUBBER STAMP. Some declared
# reasons describe rejections their schema does not perform. HOW MANY IS NOT
# WRITTEN HERE ON PURPOSE: this comment carried the word SEVEN while the gate
# below derived eight, and the binding artifact carried the same stale SEVEN, so
# a number sat wrong in two places for as long as nobody recounted. A count
# beside a list is a second source of truth for the length of the list. The
# figure is derived by `pass_proven` and printed on every run, green or red.
#
# Those fixtures are generated by `scripts/make-golden.py` and validated against
...
#: Filled by `pass_proven` on every run, read by the tests. Every figure in the
#: summary line comes from here, DERIVED, so no document holds a copy of a count
#: that can rot while the list under it moves.
PROVEN_COUNTS = {}
...
def pass_proven():
    ...
    # A REASON IS A PROMISE. A TRIPLE IS A PIECE OF EVIDENCE. They are counted
    # separately because they are not the same quantity and one promise can be
    # demonstrated by several triples.
    #
    # THIS IS THE 2026-08-29 ACCOUNTING DEFECT, STATED WHERE IT HAPPENED. A
    # single counter named `demonstrated` was incremented once per evidence
    # triple and then printed as a count of REASONS. Several reasons in this
    # repository carry more than one triple, so on 2026-08-29 the binding set's
    # 41 schema-bound reasons were reported as 45, the summary totalled 62
    # against 58 declared, and that 62 went out in a handoff to an outside
    # reviewer as a fact about this repository. Those four figures are a dated
    # record of one day's state, not a description of this repository now - the
    # current values are derived below and printed on every run. A counting bug
    # inside the gate that exists to stop counting bugs.
    #
    # Nothing here is written down anywhere else. `PROVEN_COUNTS` below is
    # derived on every run and the summary line is formatted from it, so no
    # document has to hold a copy that can go stale - which is the same failure
    # in its documentary form, and the reason the `unenforced` count used to
    # read SEVEN in two places while the gate derived eight.
    declared_total = 0     # reasons declared across every fixture walked
    schema_reasons = 0     # of those, bound to schema validation and demonstrated
    schema_triples = 0     # named error triples those reasons actually produced
    external_total = 0     # bound to a reader defect code
    unenforced_total = 0   # recorded as NOT ENFORCED by the schema
        ...
        for pos, reason in enumerate(reasons):
            ...
            if "evidence" in reason:
                if not reason["evidence"]:
                    fails.append(
                        "P0_MALFORMED  %s reason %d declares `evidence` with no "
                        "triples in it. An empty evidence list names no failure, "
                        "so nothing asks whether the promise is kept."
                        % (fx.name, pos))
                    continue
                # ONE PROMISE, HOWEVER MANY TRIPLES CARRY IT. The reason counts
                # once and only if EVERY triple it claims fired; each triple is
                # counted where it is demonstrated. Conflating the two is the
                # defect this block was rewritten for.
                reason_kept = True
    ...
    accounted = schema_reasons + external_total + unenforced_total
    PROVEN_COUNTS.clear()
    PROVEN_COUNTS.update(declared_reasons=declared_total,
                         schema_reasons=schema_reasons,
                         schema_triples=schema_triples,
                         reader_reasons=external_total,
                         unenforced_reasons=unenforced_total,
                         accounted_reasons=accounted)

    # THE ARITHMETIC, CHECKED RATHER THAN PRINTED AND TRUSTED. Every declared
    # reason lands in exactly one of the three buckets, so on a clean run the
    # three must add back up to the declaration. If they do not, the summary
    # about to be printed is wrong about this repository, and a wrong number in
    # a green gate is how 62 reached an outside reviewer.
    #
    # Guarded on `not fails` because a P0/P2/P3 finding legitimately skips a
    # reason before it is bucketed; the identity is only meant to hold when
    # every reason was walked to the end.
    if not fails and accounted != declared_total:
        fails.append(
            "P9_MISCOUNT  %d reasons are declared and %d were accounted for "
            "(%d schema-bound, %d reader-bound, %d unenforced). Every reason "
            "lands in exactly one bucket, so this gate is miscounting itself."
            % (declared_total, accounted, schema_reasons, external_total,
               unenforced_total))

    PROVEN_NOTES.append(
        "%d declared reasons accounted for: %d bound to schema validation, %d "
        "bound to a reader code, %d recorded as NOT ENFORCED by the schema."
        % (accounted, schema_reasons, external_total, unenforced_total))
    PROVEN_NOTES.append(
        "Those %d schema-bound reasons are demonstrated by %d named error "
        "triples. A REASON IS A PROMISE AND A TRIPLE IS A PIECE OF EVIDENCE: "
        "one promise can take several triples, so the two are never "
        "interchangeable and are never added together."
        % (schema_reasons, schema_triples))
    PROVEN_NOTES.append(
        "A GREEN PROVEN MEANS EVERY DECLARED PROMISE IS ACCOUNTED FOR. IT DOES "
        "NOT MEAN EVERY PROMISE IS ENFORCED: %d reasons are recorded as gaps "
        "the schema does not close, and they are still gaps on a green run."
        % unenforced_total)
    return (not fails), fails
...
def _break_proven(tmp):
    ...
    C11 known-bad promises a distinct failure per declared reason; after the
    reduction only the `bundle_kind` const survives, and PROVEN must name every
    other promise one by one. No count is written here: the surviving-versus-
    lost split is derived from the binding artifact in
    `tests/test_transfer_contract_registration.py`, which is where the exact
    number is asserted.
