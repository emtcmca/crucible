> # SUPERSEDED 2026-08-23. DO NOT SIGN THIS SHEET.
>
> **Replaced by:** `docs/proof/cartographer-adk-ratification.md`, which is bound to a
> different proposal set with a different digest.
>
> **Why:** this sheet's own section 5, *"`UNCLASSIFIED` is standing in for `INERT`, and
> they are not the same thing"*, named a gap in the Cartographer's output vocabulary and
> deliberately left it open — *"Adding `INERT` to the prompt after seeing which rows would
> move is tuning the instrument to the reading. It is a real gap, it is named here, and it
> is Eric's to rule on."* **Eric ruled on 2026-08-23:** add `INERT` as a positive assertion
> of the empty capability set. Lane `lane/cartographer-inert` changed the prompt once,
> re-ran all twelve, and reported what came back.
>
> **Nothing here was ever ratified.** This sheet was UNSIGNED for its whole life, so no
> manifest entry, no policy rule and no measured number depends on it. Superseding it
> costs nothing and invalidates nothing.
>
> **Kept, not deleted, and it is the more interesting half of the pair.** The successor
> run changed four of twelve rows against this one, and one of those rows —
> `generate_qr_code` — cites the **identical docstring span in both runs** and reaches
> `CAP_MOVES_MONEY` here and `INERT` there. That is evidence about what a citation
> constrains and what it does not, and it exists only while both readings do. Read this
> sheet's rows next to the successor's; do not read either alone.
>
> **The run this sheet records** is `docs/proof/cartographer-live-run-2026-08-22.json`,
> also superseded in place and also kept.
