...
- **The README `Observed` column is still empty and the README's Status section predates the batch.** *(opened 2026-08-25)* `README.md:362-376` is thirteen rows of an em-dash placeholder, and `README.md:72` reads *"As of 2026-08-24"* and describes the ten-run 08-24 batch, not the sixty-run 08-25 one. Filling it requires the k=1 label and the SEP-BY split beside every figure, which the batch records already carry. **Blocks [30D]:** the judge path sends a reader to Results first.
