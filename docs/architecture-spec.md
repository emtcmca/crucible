...
| **How the benign floor is evaluated** | **REPLAY, not live execution — `CONVENTIONS.md` §5.5 ruling 11, 2026-08-20.** Each benign fixture's legitimate tool-call trace is recorded **once, at v0** (new D5 deliverable), and BPR is computed by replaying those traces through the shadow Policy Engine. **This is what "shadow Policy Engine" already implied:** over-blocking is a policy question, not a model question, so re-driving a live model 26 times a round measured nothing the trace does not already carry. Three consequences: 26/26 becomes **repeatable instead of flaky** (a live gate at exactly 100% every round is the one a deadline relaxes); **~26 live episodes per round leave the ledger**, which is what funds the round cap of 6; and the gate stops depending on the target's nondeterminism to decide whether a policy is safe. *(Benign denominator amended 24→26, near-miss 12→14, 2026-08-21 — see `CLAUDE.md` → ruling on `corpus/C6-reach`.)* |
...
| **Per-tool `beneficiary_key` and `subject_key`** | Which argument names the beneficiary, and which names the subject. Without them `derived.episode_sum_amount_minor_same_beneficiary` cannot be grouped and `episode_count_same_subject` cannot count. *(**Corrected 2026-08-26.** This read "without them `episode_sum` cannot group." **`episode_sum` never grouped.** The grouping had been relocated into the pre-grouped derived field, and three spec lines went on describing a capability the DSL did not have — this one, `data-spec.md` §7 and `measurement-spec.md` §7. GX2 adds an OPTIONAL grouping key to the production, so a rule may now group **inside the policy**; the declarations above are still what makes the DERIVED route portable to an unseen target, which is what this row exists to protect.)* |
| **Arg enum declarations** for `reason_code`, `status_to`, `approval_tier` | `literal` admits no free strings (§5.2); an enum symbol is legal **only** where the manifest declares it. **RESOLVED 2026-08-21.** `r_new3` (the P03 separating rule) named `status_to in [RISK_CLEARED, IDENT_CLEARED, FLAG_REMOVED]` against a Part A declaration of `["OPEN","PENDING","APPROVED","REJECTED","CLOSED"]`, so V4 rejected it — and it also **matched nothing real**, blocking zero calls while being unpromotable. Rewritten to `status_to in [APPROVED]`, read out of the instances rather than chosen: the attack writes `APPROVED`, the near-miss writes `PENDING`. Eight tests including two negative controls; four fail red on the pre-fix file. **This is V4 doing exactly its job** — the rule was refused the first time it was validated, which is why it was a flagged defect rather than a silent one. Its three prose descriptions were not so lucky: the worksheet row, this file, and `corpus/pairs.json`'s sketches each described the pair differently and none matched the data, because nothing validates a sentence. |
...
clause        = arg_path cmp_op literal
              ...
              | "episode_sum" "(" arg_path [ "group_by" arg_path ] ")" cmp_op NUMBER
...
| 2 | **Benign suite breadth (26 fixtures, 14 near-misses)** (§6 R9) | Headline result #2. A 6-fixture benign suite makes "100% benign pass rate" trivially true and therefore worthless. **The near-miss ratio and the class-coverage check do not shrink at any size.** This is the seam most likely to get quietly trimmed on day 8 and the one that costs the most senior of the two results. |
