...
> **THE TABLE BELOW LISTS NINE. THERE ARE TEN.** `contracts/MANIFEST.json` reports
> `contract_count: 10` (C1–C10) at `spine_version: 12`. **C10 — `objective_set.schema.json`,
> the DEFINITION OF BREACH** — was added 2026-08-20 by **ruling 31**, and its instance
> `contracts/objective_set.v1.json` was hash-locked at **`19493e53a6d79d0b`** on 2026-08-22
> (ruling 44, `docs/proof/d3-objective-set-freeze.json`). §5 of this file already knows C10
> exists; this table was never carried with it. **Take the count from `hash-contracts.py`'s
> file list, never from this page** — §5 says exactly that, and this table is why it has to.
...
**Every pair carries a SEP-BY label as it is authored** — `policy` (the predicate differs on the two sides) or `oracle` (the predicate is identical; the approval oracle decides). **Target split 18 / 4. If oracle-separated reaches parity with policy-separated, STOP AND REPORT**  *(**Measured 2026-08-22: 21 policy / 3 oracle**, over 24 counted pairs with 3 cut. Off target, and off it in the safe direction — further from parity, not closer. Reported rather than absorbed; not a stop condition.)* — at parity, half the headline is a statement about a scripted oracle the builder wrote (ruling 17).
...
**Scope:** the replay viewer (**reads only from disk, no credentials**) · architecture diagram · README with the Judge-path block · the ADRs (**12 when this lane was scoped; 18 on disk as of 2026-08-22, ADR-0001..ADR-0018** — the count is read from `docs/adr/`, never from this line) · proof captures · video assets.
