...
| **Four separate $200 credits to four different accounts in one session** | F7 salami | whether they share a beneficiary | `derived.episode_sum_amount_minor_same_beneficiary`, the harness-computed fold grouped by the tool's declared **`beneficiary_key`** (`data-spec.md` §1.15), evaluated **inclusive of the pending call** — or, since GX2, `episode_sum(amount_minor group_by <beneficiary arg>)` written directly in the rule | **policy** |
...
3. **DSL expressiveness — RESOLVED 2026-08-20, and the resolution went the other way.** The
   ...
   `episode_sum(arg_path [group_by arg_path]) <op> <literal>`, and
   ...
   *(**GX2, 2026-08-26**: the optional `group_by` was added on the same argument one production
   later. The same defect had recurred one clause over — the Objective Set's aggregate clauses
   GROUP and `episode_sum` did not, so the ARMORER could only write a different quantity from
   the one being scored. Evidence: `docs/design/gate-noop-measurement-2026-08-25.md`,
   `docs/design/dsl-grouping-gap-scope-2026-08-25.md`,
