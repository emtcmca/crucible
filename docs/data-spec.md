...
|---|---|
| **Per-tool `beneficiary_key`** | Which argument names the beneficiary. Without it `derived.episode_sum_amount_minor_same_beneficiary` cannot be **grouped**, and the F7 near-miss (four $200 credits to four *different* accounts) is indistinguishable from the F7 attack (four to one) *(**Corrected 2026-08-26.** This read "without it `episode_sum` cannot group." The DSL's `episode_sum` never grouped — the grouping lives in the derived field this key defines. GX2 has since added an OPTIONAL `group_by` to the production, so a rule may now group inside the policy as well; the declaration is still what makes the DERIVED route portable to an unseen target.)* |
...
| 11 | **Demo UI** | `crucible-ui` | Cloud Run | `datastore.viewer` (**read-only**); `objectViewer` on policies; `bigquery.jobUser`+`dataViewer` on telemetry; `cloudtrace.user` | **No write anywhere.** No sealed bucket or dataset — the UI reads sealed results only from the redacted summary. **The UI has no declared path to display sealed prompts** (amended 2026-08-30 from "a screen-share cannot leak the sealed prompts": the mechanism supports the absence of a path, not the absolute) |
...
4. **The UI holds no sealed-corpus read path**, so it has no declared path to display sealed prompts. *(Amended 2026-08-30. This read "a screen-share cannot leak sealed prompts even by accident" — an absolute the mechanism does not support: it constrains what the UI can fetch, not what else is on the screen.)*
